...
namespace ssapj.GetFileInformationFromBtwFile
{
    public class GetFileInformationFromBtwFile : IDisposable
    {
        ...
        public async ValueTask<bool> IsBtwFileAsync(CancellationToken cancellationToken = default)
        {
            ...
            await this._fileStream.ReadAsync(this._bufferForHeader, 0, this._bufferForHeader.Length, cancellationToken).ConfigureAwait(false);
        ...
        public async ValueTask<IEnumerable<string>> GetBtwFileInformationAsync(CancellationToken cancellationToken = default)
        {
            ...
            await this._fileStream.ReadAsync(buffer, 0, buffer.Length, cancellationToken);
        ...
        public async ValueTask<IEnumerable<string>> GetBtwFileInformationAsync(Encoding encoding, CancellationToken cancellationToken = default)
        {
            if ((await this.IsBtwFileAsync()) == false)
            {
                throw new InvalidOperationException("This file is not a btw file.");
            }
            ...
            return SliceAtCrlf(bufferForFileInformation.AsSpan(0, length), encoding);
        }
        ...
        private static IEnumerable<string> SliceAtCrlf(ReadOnlySpan<byte> readOnlySpan, Encoding encoding)
        {
            ...
            for (var i = 1; i < readOnlySpan.Length - 1; i++)
            {
                ...
                stringList.Add(encoding.GetString(readOnlySpan.Slice(start, i - start).ToArray()));
                start = i + 2;
            }

            stringList.Add(encoding.GetString(readOnlySpan.Slice(start).ToArray()));
